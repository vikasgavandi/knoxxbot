...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function saveUserToServer() {
        ...
        addToRequestQueue(() => {
            return fetch('http://localhost:3002/set-user', {
                method: 'POST',
                headers: {
    ...
    function loadChatHistory() {
        ...
        // Add to request queue instead of executing immediately
        addToRequestQueue(() => {
            return fetch(`http://localhost:3002/user/chats?sessionId=${sessionId}`)
                .then(response => response.json())
                .then(data => {
    ...
    function toggleChatMinimize() {
        ...
        if (isMinimized) {
            minimizeChatBtn.innerHTML = '<i class="fas fa-expand"></i>';
            minimizeChatBtn.title = "Expand Chat";
        } else {
            minimizeChatBtn.innerHTML = '<i class="fas fa-minus"></i>';
            minimizeChatBtn.title = "Minimize Chat";
        }
    }
});